...
`ExternalDrift` 表示当前文件身份无法与原生或安装记录对应。v1.2.1 修复了原生字体使用完整注册路径时的误报；仍出现时，运行 `aes.cmd status` 并保留完整诊断输出。不要删除原始备份。诊断信息可帮助排查注册路径、安装记录缺失，以及 Windows 更新或其他工具造成的字体变化。
