...
try {
    ...
    $applied = $actual -eq ([string]$journal.pendingOutputHash).ToLowerInvariant()
    if ($applied) { [AppleEmojiSwitcher.NativeTransaction]::SetFileSecuritySddl($target, [string]$journal.originalSddl) }
    $state = @{ schemaVersion = 1; status = if (-not $applied) { 'NotApplied' } elseif ($journal.operation -eq 'Restore') { 'Original' } else { 'Installed' }; installedOutputHash = if ($journal.operation -eq 'Restore' -or -not $applied) { [string]$prior.installedOutputHash } else { [string]$journal.pendingOutputHash }; pendingOutputHash = $null; pendingOperation = $null; operationId = $OperationId; finalizedUtc = [DateTime]::UtcNow.ToString('o'); renderVerificationPending = ($applied -and $journal.operation -ne 'Restore'); reason = if ($applied) { '' } else { 'The queued boot-time move was not applied; original inode security was restored.' } }
